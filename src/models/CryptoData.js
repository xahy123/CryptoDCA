...
// 创建一个加密货币数据的默认结构
export const createCryptoData = (id, name, symbol, options = {}) => ({
  id,                // 唯一标识符
  coingeckoId: options.coingeckoId || '', // CoinGecko 币种ID
  image: options.image || '',
  marketCapRank: options.marketCapRank ?? null,
  name,              // 币种名称
  symbol,            // 币种符号
  currentPrice: options.currentPrice || 0, // 当前价格
  averageCost: 0,    // 平均成本
